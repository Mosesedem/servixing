  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `/api/parts/search?q=${encodeURIComponent(query)}&brand=${encodeURIComponent(deviceBrand)}`,
      )
      ...
      {searched && (
        <div className="space-y-3">
          {parts.length === 0 ? (
            ...
            <>
              <p className="text-sm font-semibold mb-3">{parts.length} parts found</p>
              {parts.map((part) => (
                <div key={part.id} className="border border-border rounded p-4 hover:bg-muted/50 transition">
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-sm font-semibold text-orange-600">${part.price.toFixed(2)}</p>
                    </div>
